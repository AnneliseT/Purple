function Purchases() {
    ...
    function makeItem(name, price, category) {
        return {
            name: name,
            price: price,
            category: category,
...
//adds an item object to the item list (cart)
    ...
    function createEntry() {
        ...
        if (name !== "" && validateNumber(value) && category !=="Select Category" && category !=null) {
            //set content for entry
            var item = makeItem(name,value,category);
            addToCart(item);
            $item.html(`${item.name}, ${'$' + item.price},${item.category}`);
            ...
            $item_name.val("");
            $item_value.val("");
            $item_category.val("Select Category");
